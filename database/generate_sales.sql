...
WITH parameters AS (
    SELECT CAST(round(1000*RAND()) AS integer) ids_count, DATE '2022-01-01' start_date, DATE '2022-04-30' finish_date
  )
  SELECT  DATE_FROM_UNIX_DATE(CAST(start + (finish - start) * RAND() AS INT64)) random_date, model_id, CAST(round(99*RAND()) AS integer) reference_id
