...
def get_models_by_word(query):
    ...
    for model in results:
        models[model["model_id"]] = Model(model["model_id"], model["name"], model["year"], model["brand"], [ Used_Part(Parts(part["reference_id"], None, part["cont_part"]), part['total_ref_used']) for part in model['references_used'] ], random.choice([True, False]))
    return models
...
def get_stock():
    query_job = client.query(
    """
    SELECT * FROM `bot-testing-345117.hackupc2022.warehouse_v4` LIMIT 100"""
    )
    stock = {}
    results = query_job.result()
    for part in results:
